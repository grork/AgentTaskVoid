...
The recorder leg is complete. Event-to-state implications below are grounded
in the real captures rather than the earlier docs-only hypotheses.
...
fall through when they emit no output. `preToolUse` has one important failure
caveat: a command hook crash or nonzero exit is fail-closed and denies the tool
call, while a timeout is fail-open. The compiled recorder's fixed argv and
stdout-silent behavior therefore remain load-bearing during this supervised
scratch-repo capture.
...
`sessionEnd` lands before process exit. Unlike Claude Code's observer posture,
current Copilot command hooks are synchronous; the template deliberately
...
5. **Copilot repeats system notifications as submitted prompts.** Both
   ...
   `working --goal -`. This independently reproduces the post-phase-19 Claude
   Code goal-pollution loose end on a second host.
...
7. **Ready and cleanup must be gated.** Main `agentStop` and
   ...
   active. This is the Copilot equivalent of phase 19D's premature Claude
   `Stop`. A translator that wants correct background fan-out needs its own
   tiny per-session correlation/count state, or it must deliberately degrade
   by ignoring these completion signals and relying on later cleanup.
   Interactive `sessionEnd.reason:"user_exit"` was a reliable terminal signal.
...
- **Decision (2026-07-16): ship the handler-local high-fidelity bridge.** The
  production plugin under `integrations/copilot-cli/` hashes the exact task
  prompt into a short-lived pending record, atomically claims it when the
  child repeats that prompt under `call_*`, and retains only the resulting
  `call_* -> parent/task` mapping through child completion. It never reads the
  internal Copilot transcript, never stores raw prompts, and refuses ambiguous
...
This closes the recorder-to-production loop: the correlation bridge is proven
against real hook ordering, real child-card routing, the real taskbar, and real
cleanup rather than only the stub-atv harness.

One live limitation was explicitly retained by operator decision
(2026-07-16): a child-raised permission notification carries only the parent
session id, and Copilot publishes no permission-completed hook. The plugin
therefore blocks the parent locus; approving the prompt does not clear it until
later parent activity or final Ready. Claude's better child attribution still
only clears at `PostToolUse` (tool completion), so widening Copilot's
synchronous post-tool hooks would not solve a long-running build and would add
cost to every tool. No timer, internal-transcript read, or optimistic clear was
accepted.
