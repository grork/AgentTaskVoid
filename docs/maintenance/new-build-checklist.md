...
the default test run — it's genuinely "dark matter": low-probability of
changing, expensive to re-verify by hand, and safe to defer until there's a
concrete reason to suspect drift (a new Windows 11 build/Insider ring landing
on a dev or dogfood machine, a bug report that smells like a rendering
regression, or a scheduled periodic check if one is ever set up). **Low
priority by design** — this doc exists so that *when* someone does the
check, they know exactly what to run and where the result has to be updated.
...
recorded, and everything downstream (the validator, the fake, this checklist)
...
wrong vs. actually crashes `explorer.exe`. Full matrix + the exact
...
The safe subset this codebase actually uses is encoded as data in
...
worth adding to `SafeCells` (never required to use it, but no longer worth
guarding against).

**If something changed:** update `SafeCombinationMatrix.SafeCells` AND
...
matrix (it currently says the fake deliberately does *not* model
Shell-render-only behavior — re-read that note before deciding whether a
crash-matrix change should ever change the fake, since INFRA-10 already
decided it shouldn't).
...
section — this is prose documentation, not a data table, so there's no
second file to keep in sync, but re-read whether `IconService`'s per-handle
icon design (which exists *because* of this fact) still makes sense.
...
to mimic exactly four real-platform behaviors — no more, gated by "a specific
logic test would test the wrong thing without it." Three of the four have
fully automated confirming checks (phase 02/03 unit + adapter tests, already
part of the default suite). **Two are genuinely manual** — this checklist's
actual "dark matter":
...
— excluded from the default adapter run (gated off, phase 03 precedent).
Empirical baseline (Windows 11 26100, 2026-07-02): 4 processes × 100 creates
each, unlocked, kept only 37/400; the same test through the mutex kept
400/400. Re-running should reproduce "well under 400 survive unlocked,
400/400 locked" — the exact surviving count isn't the contract, only that
loss still happens without the mutex and doesn't with it.

**If something changed** (e.g. a new Windows build somehow serializes writes
at the OS/API level and the unlocked run now keeps ~400/400 too): re-read
...
ERGO-2 sweep) to notice and `Remove()` it, or it lingers forever. This is the
one fidelity promise with **no automated check at all** (per the doc: "MANUAL
'dark matter' only — no automated harness can drive a real Shell click").
...
`docs/windows-ui-shell-tasks/AppTaskInfo.md`'s `HiddenByUser` row AND
...
clean `true`/`false` or the same documented COM exception on the new build,
not some third failure shape the wrapper doesn't handle.
