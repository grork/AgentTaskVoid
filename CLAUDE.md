...
A check that needs the real taskbar registers a throwaway identity you control — the `-reltest` variant (alias `atv-reltest`, [`docs/release.md`](docs/release.md) §3) or a one-off `winapp run` registration — never the operator's `atv` (see "Package identity" above). Registering guarantees the just-built version. Drive and observe cards across as many invocations as needed, then remove the package (DIST-9) by its exact Name — `Remove-AppxPackage`, never a bare `*Codevoid.AgentTaskVoid*` wildcard, which would also catch the retail, dev, and test pools. A live watchdog holding the exe can make Windows defer the removal or a re-registration (`ERROR_PACKAGES_IN_USE`) until it exits (DIST-6).

`winapp run --unregister-on-exit` unregisters the package once the process exits, but no MSBuild property passes it through `dotnet run` — the WinApp targets map only `--with-alias`, `--no-launch`, `--debug-output`, and `--args` — so it needs a direct `winapp run` invocation. Unregistering drops the cards immediately, so it can't observe a persistent card or hold state across invocations — it fits only a one-process create-and-assert check that leaves no residue.
